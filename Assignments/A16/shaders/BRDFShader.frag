...
vec3 Oren_Nayar_Diffuse_BRDF(vec3 L, vec3 N, vec3 V, vec3 C, float sigma) {
	...
	vec3 l = C*clamp(dot(L,normalize(N)),0.0,1.0);
	float sigmai = acos(dot(L,normalize(N)));
	float sigmar = acos(dot(V,normalize(N)));
	float alpha = max(sigmai,sigmar);
	float beta = min(sigmai,sigmar);
	vec3 vi = normalize(L - (dot(L,normalize(N))*N));
