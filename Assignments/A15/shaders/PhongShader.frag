...
vec3 point_light_color(vec3 pos) {
	// Point light color
	float base = gubo.coneInOutDecayExp.z/length(gubo.lightPos - pos);
	return gubo.lightColor * pow(base, gubo.coneInOutDecayExp.w);
}
...
vec3 spot_light_color(vec3 pos) {
	// Spot light color
	float base = gubo.coneInOutDecayExp.z/length(gubo.lightPos - pos);
	vec3 first = gubo.lightColor * pow(base, gubo.coneInOutDecayExp.w);
	float second = clamp((dot(normalize(gubo.lightPos - pos), gubo.lightDir) - gubo.coneInOutDecayExp.x)/(gubo.coneInOutDecayExp.y - gubo.coneInOutDecayExp.x), 0.0, 1.0);
	return first * second;
}
